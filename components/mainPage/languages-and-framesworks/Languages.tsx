...
const pictureSwitch = (language: string) => {
  ...
  switch (language) {
    case "TypeScript":
      return (
        <SiTypescript
          className={`${style} text-blue-500 bg-white rounded-md`}
        />
      );
    case "JavaScript":
...
const Languages = (props: Props) => {
  ...
  const mapLanguages = languages.map(({ id, language, link }) => {
    return (
      <div
        key={id}
        className="flex flex-row items-center m-5 bg-onyx text-cadetGray p-4 rounded-md hover:-translate-y-1 hover:shadow-md hover:shadow-onyx"
      >
        <Link href={link}>
          <h2>{language}</h2>
          <div>{pictureSwitch(language)}</div>
        </Link>
      </div>
    );
  });
